...
void test_extend_nanafy_list (){
	int *listseq = malloc(sizeof(int) * LIST_LENGTH);
	test(listseq != NULL);
	...
	// *extend
	test(extend_nanafy_int_list(3, &list) == 0);
	// *add
	{
		size_t index;
		test(has_nanafy_int_list_margin(1, &list) == true);
		test(add_nanafy_int_list(4, &list, &index) == 0);
		test(index == 3);
		test(nanafy_int_list_length(&list) == 4);
	}
	// *get
	{
		int value;
	...
	{
		int value;
		test(get_nanafy_int_list(3, &list, &value) == 0);
		test(value == 4);
	}
	// *free
	free_nanafy_int_list(&list);
}
