...
void test_extend_nanafy_array (){
	int *arrayseq = malloc(sizeof(int) * ARRAY_LENGTH);
	test(arrayseq != NULL);
	...
	test(set_nanafy_int_array(4, ARRAY_LENGTH +7, &array) != 0); // always fail!
	// *get
	{
		int value;
	...
	{
		int value;
		test(get_nanafy_int_array(ARRAY_LENGTH +7, &array, &value) != 0); // always fail!
	}
	// *get
	for (size_t index = 3; index < ARRAY_LENGTH; index++){
		int value;
		test(get_nanafy_int_array(index, &array, &value) == 0);
		test(value == 0);
	}
	// *extend
	test(extend_nanafy_int_array(0, 8, &array) == 0);
	// *set
	test(set_nanafy_int_array(4, ARRAY_LENGTH +7, &array) == 0);
	// *get
	{
		int value;
		test(get_nanafy_int_array(ARRAY_LENGTH +7, &array, &value) == 0);
		test(value == 4);
	}
	// *get
	for (size_t index = 3; index < ARRAY_LENGTH +7; index++){
		int value;
		test(get_nanafy_int_array(index, &array, &value) == 0);
		test(value == 0);
	}
	// *free
	free_nanafy_int_array(&array);
}
